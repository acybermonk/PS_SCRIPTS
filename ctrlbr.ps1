...
# Default File Path
    $Default_FilePath = "$env:USERPROFILE\Documents\$Global:AppName"
# Write PingIt file
    $PingIt_fileText = 
'
# Ping It
...
function displayHeader{
    ...
    Write-Host -ForegroundColor Blue "Action Path  : $ActionPath"
    Write-Host -ForegroundColor Blue "Action Type  : $ActionType"
    Write-Host -ForegroundColor Blue "Tartget Path : $TargetPath"
    Write-Host -ForegroundColor Blue "Username     : $username"
    Write-Host -ForegroundColor Blue "-----------------------------------"
    Write-Host -ForegroundColor Blue "Max Attempts : $Global:tryCount"
    getDate
...
Write-Host -ForegroundColor Blue "Starting Auto Run : $Date"
...
function doIt{
    ...
    if($Global:FileList.Count -gt 0){
        ...
        Write-Host "Current Data Count : $dataCount" -Foreground Blue
    }else{
    ...
    if ($pingCheck -like "reply*"){
        ...
        try{
            ...
            if ($ActionType -eq "powershell.exe"){
                $process = Start-Process -PassThru -FilePath $ActionType -Verb RunAs -ArgumentList "-File $ActionPath $system $user $group"
            }elseif ($ActionType -eq "cmd.exe"){
                $process = Start-Process -PassThru -FilePath $ActionType -Verb RunAs -ArgumentList "/c $ActionPath $system $user $group"
            }
...
Write-Host -ForegroundColor Blue "Attempts remaining : ${Global:tryCount}"
Write-Host -ForegroundColor Blue "*---------------------------*"
    if ($Global:tryCount -gt 0){
        if (($Global:dataCount - $c) -gt 0){
            ...
            $Global:dataCount = 0
            $minutes = $sleep / 60
            Write-Host -ForegroundColor Blue "*PREPARING TO LOOP DATA*"
            Write-Host -ForegroundColor Blue "Sleeping for ${minutes} minutes. Please wait..."
...
'#PSONE TEMPLATE
# *********************************
# Control Bridge Template PS Script
# *********************************
...
###################
    ...
    # Log
    $AppLogFile = "CtrlBR_Err.log"
    $AppLogRoot = "\\lewvdeployops\E$\CtrlBrLogs"
    $Global:LocalApplogPath = "$AppLogRoot\$AppLogFile"

    function Applogwrite{
        param([parameter(Mandatory)][string]$Applogstring)
        $AppLogTime = Get-Date -Format "MM/dd/yyy HH:mm:ss"
        Add-Content $Global:LocalApplogPath -Value "[LOGGING]$AppLogTime --- $Applogstring"
    }

    # Check and verify PSRemoting is enabled for script
    $ChkPSRemoteStatus = Test-WSMan
    if (-not ($ChkPSRemoteStatus)){
        Write-Host "** PSRemoting Disabled **" -ForegroundColor Yellow
        Write-Host "Attempting to Enable PSRemoting"
        Enable-PSRemoting -Confirm:$false -Force -ErrorAction SilentlyContinue -WarningAction SilentlyContinue | Out-Null
        $ChkPSRemoteStatus = Test-WSMan
        if (-not ($ChkPSRemoteStatus)){
            Write-Host -Message "PSRemoting could not be enabled"
        }else{
...
##########################
    ...
    Invoke-Command -ComputerName $systemName -ErrorVariable +errVar -ScriptBlock{
...
# Wait Check
#################### 
    Write-Host "--Waiting for verification--"
    Pause
    Exit
'
...
'REM BAT TEMPLATE
@echo off
SET AppName="ControlBridge"
C:
cd %userprofile%\%AppName%\PsTools
REM PSexec can be called from this location
...
$Version_Label = New-Object $Label_Object
$Version_Label.Text = "v$Global:AppVer"
$Version_Label.Font = New-Object System.Drawing.Font("Calibri",9,[System.Drawing.FontStyle]::Bold)
$Version_Label.AutoSize = $true
$Version_Label.Location = New-Object System.Drawing.Point(400,175)
